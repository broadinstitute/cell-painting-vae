...
for latent_name in latent_names:
    if latent_name == "pca":
        ...
        latent_dfs[latent_name] = pd.read_csv(f"level4Latent_{latent_name}.csv").drop("Unnamed: 0", axis = 1)
...
for latent_name in latent_dfs:
    ...
    if latent_name not in ['pca', 'complete']:
        decoder = keras.models.load_model(f"../2.train/models/level4Decoder_{latent_name}")        
    ...
    per_moa_performance_df.to_csv(f'per_moa_performance/level4_{latent_name}_moa_performance.csv')
    ...
    L2_df.to_csv(f'L2_distances/level4_L2_general_{latent_name}.tsv', sep = '\t')
